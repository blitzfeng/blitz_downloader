...
> **实现偏离（2026-08-11）**：上面这套界面已被替换，本节仅保留为设计过程记录。
>
> 实际实现是 Material Design 组件（`TextInputLayout` + `MaterialAutoCompleteTextView` 的
> exposed dropdown menu，按钮为 `MaterialButton`），且**基准标签提到对话框顶部单独一行**、
> 不再在每条规则行里重复显示：
>
> ```
> [基准标签: 美女        ▾]
...
> ```
>
> 原因有两条：① Material 的外框 + 下拉图标比平台 `Spinner` 占更多横向空间，三列并排
> 在手机对话框里每格只剩一个字；② 基准标签对所有行本来就是同一个值，提到顶部后
> 「删掉首行导致基准静默回退成旧值」那一类问题从结构上消失。
> 「没选」由空文本 + 浮起的 hint 表示，不再需要「请选择」占位项。
> 运算符文案 `或（取并集）` 简化为 `或`——括号里的解释由底部实时表达式承担。
>
> 求值语义、取数链路、`TagQuery` 的接口一行未改，本文档其余部分仍然有效。
