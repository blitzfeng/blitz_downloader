...
- 点赞浏览图集缓存必须保存完整图片列表及逐图实况视频配对（v27 的 photoMediaJson），恢复为 imageUrls/imageVideoUrls，不可只保存首图。点击缓存条目预览或勾选时先通过作品详情校正类型并回写媒体缓存，再进入图集选图或视频预览；旧缓存无需重置。图集部分选择在下载前重新解析后仍必须传递 selectedImageIndices。
