...
| 标签数量（0..5+，**可多选取并集**） | `Set<ManageTagCountFilter>`（空集 = 不筛选） | 只在内存（`postProcess`） |
...
- `watched`（是否已看过）只由**管理页进入视频播放页**置位：`ManageVideoFragment` 把 `awemeIds` 随 `createListFileIntent` 传给播放页，播放页每加载一条就写库（含上下滑动切到的）。列表侧「未看过」标记的刷新分两条路：点开那条就地标掉，滑动看过的靠 `onResume` 回查——**别把其中一条删掉当冗余**。
