...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_14_15 = object : Migration(14, 15) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE downloaded_videos ADD COLUMN hasLivePhoto INTEGER NOT NULL DEFAULT 0",
                )
            }
        }
        ...
        /**
         * v21 → v22：
         * - `tags` 表新增 `enableAi`（是否参与 AI 分析，默认 1）与 `isExclusive`（子标签是否互斥单选，默认 0）。
         * - `video_tag_feedback` 表新增 `evidenceImagePath`（推断依据帧相对路径）。
         * - 保持预置标签「小沟」的一致性。
         */
        private val MIGRATION_21_22 = object : Migration(21, 22) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE tags ADD COLUMN enableAi INTEGER NOT NULL DEFAULT 1")
                db.execSQL("ALTER TABLE tags ADD COLUMN isExclusive INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE video_tag_feedback ADD COLUMN evidenceImagePath TEXT DEFAULT NULL")
                db.execSQL("UPDATE tags SET tagName = '小沟' WHERE tagName = '乳沟'")
                db.execSQL("UPDATE tags SET description = REPLACE(description, '乳沟', '小沟') WHERE description LIKE '%乳沟%'")
            }
        }
